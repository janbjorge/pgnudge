...
Read this file fully before changing anything. Settled design decisions are
summarized in the five laws below; open an issue before re-litigating any
of them.

## The five laws (non-negotiable)

These are settled constraints, not preferences. Do not trade any of them
away, even for apparently good reasons. If a task seems to require
violating one, stop and raise it first.

1. **Push only.** No polling of any kind: no `pg_logical_slot_get_changes`,
   no periodic SELECTs, no sentinel queries. The only timers permitted are
...
4. **Driver-free.** The replication transport is a hand-rolled walsender
   ...
   open since 2017) and psycopg2 is out of scope. Runtime dependency:
   scramp only. asyncpg exists solely in the dev group for the live test's
   admin connection.
...
- **Switch to pgoutput to drop the wal2json dependency.** Careful: pgoutput
  ...
  object**, which likely violates law 3. This roadmap item is conditional
  at best; treat it as an open design question, not a task.
- **Add a trigger-based emitter.** Persistent catalog objects violate
  law 3; fan-out goes through the bridge daemon.
- **Add a NOTIFY consumer transport.** pgnudge is WalFeed-only. Fan-out is
  a bridge daemon republishing via `pg_notify`; consumers LISTEN with
  whatever driver they already have.
...
tests/
  conftest.py  session-scoped PostgreSQL via testcontainers, scratch
               ...
               cert) inside the container. Env knobs:
  ...
  test_wal.py  live tests incl. the two proofs: no backfill of pre-connect
               writes, and hard abort -> pg_replication_slots EMPTY.
docs/
  temporary-slots.md  reference: logical decoding + temporary-slot
...
.github/workflows/
  ci.yml       lint (ruff + mypy), test matrix Python 3.11-3.14 x PG 16-18
               (test_decoding), one wal2json job on a CI-built image.
```

The consumer contract, coalescing semantics, and ops notes are in
`README.md`; treat it as the spec. `docs/temporary-slots.md` covers the
mechanism.
...
# Coverage (100% line+branch; keep it there.
...
dropped wal2json; error 58P01). Build one: `postgres:16` + apt
`postgresql-16-wal2json` from the PGDG repo the official image already has
configured. See the docker build step in `ci.yml`, then
...
- **No `from __future__ import annotations`.** pgnudge targets Python >=
  3.11; everything used is runtime-valid on 3.11 (`Self`, PEP 604 unions,
  `collections.abc` imports). **No PEP 695** `type` aliases or generic
  syntax, those need 3.12+; use `typing.TypeAlias` and `typing.TypeVar`
  instead.
...
- **No module-level globals.** Constants live as class attributes
  (`ClassVar`); test config lives inside functions.
- **No leading-underscore prefixes on new names.** Python has no real
  ...
  behind a prefix. Existing `_`-prefixed internals in
  `proto.py`/`engine.py`/`wal.py` predate this rule; renaming them is a
  pending refactor. Coordinate before a mass rename, and never
  underscore-prefix anything new. Module and class names and dataclass
  fields must not use underscore prefixes (internal dataclass state uses
  `field(init=False)` with a plain name). Python-mandated protocol dunders
  (`__init__.py`, `__aenter__`, `__version__`, ...) are exempt.
...
which keeps a future native implementation possible. Never break either in
...
1. **Bridge daemon** as a first-class example or subpackage: one WalFeed ->
   `pg_notify` on a channel -> plain LISTEN consumers. Zero persistent
   objects end to end (the bridge's slot dies with the bridge).
2. **Opt-in payload v2** (`schema.table:pk`): wal2json carries pk columns;
