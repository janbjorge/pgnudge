...
A study companion to [parsing-reference.md](parsing-reference.md). Where that page is the
...
Read this first to build the mental model, then use `parsing-reference.md` as the
lookup table.
...
2. Read the first 4 bytes to get `tot_len` (`<I`). Sanity-bound it: at
   least the 24-byte header, at most 1 GiB, else `WalSyncError`.
...
| *(if HAS_IMAGE 0x10)* | var | none | full-page-image header, skipped by length |
...
are handled by length, see `parsing-reference.md` for the exact bytes.)
...
### Step 6: relfilenode to `schema.table`
...
For the field-level definitions of this engine tail (`Intake` overflow,
`Coalescer`, `Debouncer`, `Event`/`Batch`/`Resync`), see
[parsing-reference.md](parsing-reference.md#convergence-everything-becomes-a-string), the
...
watch Step 0 through Step 4 scroll by.
...
- [parsing-reference.md](parsing-reference.md): the exhaustive byte-level reference for
